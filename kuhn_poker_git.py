...
import tempfile           # OS共通の一時フォルダパスを取得するために使う
...
MODEL_FILENAME = os.path.join(tempfile.gettempdir(), "kuhn_model.pkl")
# → __file__（このスクリプト自身のパス）と同じフォルダに "kuhn_model.pkl" を置く
...
TRAIN_CHUNK     =   5_000   # 進捗バーを更新する頻度


def _train_with_progress():
    trainer = KuhnCFRTrainer()  # 空のnode_mapを持つ学習器を新規作成

    st.subheader("🤖 AIを学習中...")
    st.caption("初回起動時のみ実行されます。完了後、自動でゲームが始まります。")
    bar = st.progress(0.0, text="準備中...")  # 0%の進捗バーを表示

    def on_progress(done, total):
        # 毎反復呼ばれるが、TRAIN_CHUNK ごと（と最終回）にだけバーを更新する。
        if done % TRAIN_CHUNK == 0 or done == total:
            bar.progress(done / total, text=f"学習中... {done:,} / {total:,} 回")

    trainer.train(TRAIN_ITERATIONS, on_progress=on_progress)  

    trainer.save_model(MODEL_FILENAME)  # 学習結果をpklに保存（次回以降は再学習不要にする）
    bar.progress(1.0, text="✅ 学習完了！ゲームを開始します...")
    st.rerun()  # スクリプトを再実行 → 今度はpklが存在するのでmain()がゲーム画面を表示する
...
def load_trainer():
    ...
    Streamlit はボタン押下や画面更新のたびにスクリプト全体を再実行する。
    @st.cache_resource デコレータを付けた関数は「アプリの生存期間中に
    一度だけ実行し、結果をメモリにキャッシュする」という挙動になる。
    """
    trainer = KuhnCFRTrainer()        # 空のtrainerを作成
    trainer.load_model(MODEL_FILENAME)  # pklからnode_mapを読み込んで上書き
    return trainer                     # キャッシュされ、以降の呼び出しではこの戻り値が再利用される
...
def main():
    ...
    # pklがなければ進捗バー付きで学習 → 保存 → rerun
    if not os.path.exists(MODEL_FILENAME):
        _train_with_progress()  # 学習画面を表示し、完了後にst.rerun()される
        return                   # ここで処理を終え、ゲーム画面の描画は行わない

    init_session()        # session_stateの初期化（既存値は維持）
    trainer = load_trainer()  # キャッシュされた学習済みtrainerを取得
